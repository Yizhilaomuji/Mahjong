...
void SimulationWidget::refreshPlayerUI(int player) {
    ...
    if (player == 0) {
        ...
        for (size_t i = 0; i < ps.hand.size(); ++i) {
            QPushButton *btn = new QPushButton(tileToDisplay(ps.hand[i]), this);
            btn->setFixedSize(48, 60);
            btn->setEnabled(isTurn);
            QString color = suitColor(ps.hand[i].suit);
        ...
        for (size_t i = 0; i < ps.hand.size(); ++i) {
            ...
            QString color = suitColor(ps.hand[i].suit);
            btn->setStyleSheet(QString(
                "QPushButton { background-color: %1; color: white; font-size: 11px; "
