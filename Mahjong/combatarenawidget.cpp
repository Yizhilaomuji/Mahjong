...
void CombatArenaWidget::setupUI() {
    ...
    handScroll->setMaximumHeight(90);
    handScroll->setStyleSheet("QScrollArea { border: 1px solid #555; border-radius: 4px; }");
...
QGroupBox *CombatArenaWidget::createOpponentBox(int seatIndex) {
    ...
    m_oppDiscardLabels[player]->setMinimumHeight(36);
...
void CombatArenaWidget::onPeekOpponent(int player) {
    ...
    for (const auto &t : ps.hand) {
        QPushButton *btn = new QPushButton(tileToDisplay(t), dlg);
        btn->setFixedSize(46, 56);
        btn->setEnabled(false);
        QString color = suitColor(t.suit);
        btn->setStyleSheet(QString(
            "QPushButton { background-color: %1; color: white; font-size: 13px; "
            "border: 1px solid #555; border-radius: 4px; }").arg(color));
...
void CombatArenaWidget::refreshOpponentUI(int player) {
    ...
    // 查看手牌按钮 — 实战模式禁止，模拟模式始终可点
    m_oppPeekButtons[player]->setEnabled(m_mode == ArenaMode::AISimulation);
...
void CombatArenaWidget::refreshHandUI() {
    ...
    for (size_t i = 0; i < ps.hand.size(); ++i) {
        Tile tile = ps.hand[i];
        QPushButton *btn = new QPushButton(tileToDisplay(tile), this);
        btn->setFixedSize(50, 65);
...
void CombatArenaWidget::refreshSuggestions() {
    ...
    if (shanten == -1) {
        ...
        if (!waits.empty()) {
            ...
            m_suggestionBox->append(QString("听 %1 张: %2").arg(waits.size()).arg(waitStr));
        }
        return;
    }
    ...
    if (!suggestions.empty()) {
        ...
        for (const auto &pair : suggestions) {
            if (pair.second.size() > maxWaits) maxWaits = pair.second.size();
        }
        ...
        for (const auto &pair : suggestions) {
            ...
            bool isBest = (pair.second.size() == maxWaits);
