    ...
    : QWidget(parent), m_mode(mode)
{
    ...
    // 初始化 AI 手牌区域
    for (int i = 0; i < 4; ++i) {
        m_oppHandScrolls[i] = nullptr;
        m_oppHandInners[i] = nullptr;
        m_oppHandInnerLayouts[i] = nullptr;
    }
...
QGroupBox *CombatArenaWidget::createOpponentBox(int seatIndex) {
    ...
    box->setStyleSheet(
        ...
        "border-radius: 6px; padding: 6px 8px 6px 8px; }");
    ...
    titleLabel->setStyleSheet(
        "font-weight: bold; font-size: 13px; color: #222; padding: 2px 0;");
    headerRow->addWidget(titleLabel);
    headerRow->addStretch();
    m_oppDingqueLabels[player] = new QLabel("定缺: --", this);
    m_oppDingqueLabels[player]->setStyleSheet("font-size: 11px; color: #555;");
    headerRow->addWidget(m_oppDingqueLabels[player]);
    ...
    m_oppMeldLabels[player]->setStyleSheet("font-size: 11px; color: #333;");
    layout->addWidget(m_oppMeldLabels[player]);

    // 手牌 — 可滚动区域，内嵌牌图片
    QScrollArea *handScroll = new QScrollArea(this);
    handScroll->setWidgetResizable(true);
    handScroll->setMinimumHeight(62);
    handScroll->setMaximumHeight(72);
    handScroll->setStyleSheet(
        "QScrollArea { border: 1px solid #e0d5c0; border-radius: 4px; "
        "background-color: #fffdf5; }");
    QWidget *handInner = new QWidget(this);
    QHBoxLayout *handInnerLayout = new QHBoxLayout(handInner);
    handInnerLayout->setSpacing(1);
    handInnerLayout->setContentsMargins(3, 2, 3, 2);
    handInnerLayout->addStretch();
    handScroll->setWidget(handInner);
    layout->addWidget(handScroll);

    m_oppHandScrolls[player] = handScroll;
    m_oppHandInners[player] = handInner;
    m_oppHandInnerLayouts[player] = handInnerLayout;
    ...
    m_oppDiscardLabels[player]->setMinimumHeight(30);
    m_oppDiscardLabels[player]->setStyleSheet("font-size: 11px; color: #444; padding: 2px;");
    layout->addWidget(m_oppDiscardLabels[player]);

    // "查看手牌"按钮 (游戏结束后弹出详细手牌窗口)
    QPushButton *peekBtn = new QPushButton("查看手牌", this);
    peekBtn->setStyleSheet(
    ...
    connect(peekBtn, &QPushButton::clicked, this, [this, player]() {
        onPeekOpponent(player);
    });
    layout->addWidget(peekBtn);
...
void CombatArenaWidget::refreshOpponentUI(int player) {
    ...
    // AI 手牌显示
    if (player != 0 && m_oppHandScrolls[player]) {
        QHBoxLayout *handLayout = m_oppHandInnerLayouts[player];
        // 清空旧手牌
        while (handLayout->count() > 0) {
            QLayoutItem *item = handLayout->takeAt(0);
            if (item->widget()) item->widget()->deleteLater();
            delete item;
        }

        if (m_mode == ArenaMode::AISimulation
            && m_engine->phase() != GameEngine::Finished
            && !ps.hasWon) {
            // AI 模拟模式: 显示真实牌面
            for (const auto &t : ps.hand) {
                ...
                btn->setEnabled(false);
                btn->setStyleSheet(
                    "QPushButton { background-color: #fafafa; border: 1px solid #ddd; "
                    "border-radius: 3px; }");
                handLayout->addWidget(btn);
            }
        } else if (ps.hasWon) {
            QLabel *wonLabel = new QLabel("已胡牌 ✓", this);
            wonLabel->setStyleSheet("color: #cc4444; font-weight: bold; font-size: 12px;");
            handLayout->addWidget(wonLabel);
        } else {
            // 实时模式: 只显示张数
            QLabel *countLabel = new QLabel(
                QString("%1 张 (隐藏)").arg(ps.hand.size()), this);
            countLabel->setStyleSheet("color: #888; font-size: 12px;");
            handLayout->addWidget(countLabel);
        }
        handLayout->addStretch();
    }
...
void CombatArenaWidget::initBackgroundMusic() {
    ...
    QStringList searchPaths = {
        QDir::cleanPath(QCoreApplication::applicationDirPath() + "/../music/dushen.mp3"),
        QDir::cleanPath(QCoreApplication::applicationDirPath() + "/../../music/dushen.mp3"),
        QDir::cleanPath(QCoreApplication::applicationDirPath() + "/music/dushen.mp3"),
        QDir::cleanPath(QStandardPaths::writableLocation(QStandardPaths::MusicLocation) + "/dushen.mp3"),
    };
